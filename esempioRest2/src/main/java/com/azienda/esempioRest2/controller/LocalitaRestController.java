package com.azienda.esempioRest2.controller;
...
import com.azienda.esempioRest2.service.LocalitaService;
...
public class LocalitaRestController {
	...
	public ResponseEntity<Localita> getById(@PathVariable("id") Integer id){
		try {
			...
			Localita l=localitaService.ricercaById(id);
	...
	public ResponseEntity<Localita> getByNome(@PathVariable("nome") String nome){
		try{
			...
			Localita l=localitaService.ricercaByNome(nome);
			HttpStatus httpStatus= l!=null ? HttpStatus.OK : HttpStatus.NOT_FOUND;
			return new ResponseEntity<Localita>(l, httpStatus);
	...
	public ResponseEntity<List<Localita>> getByTempUnder(@PathVariable("temperatura") Float temperatura){
		try{
			...
			List<Localita> locFredde=localitaService.ricercaByTemperaturaUnder(temperatura);
	...
	public ResponseEntity<List<Localita>> getByTempOver(@PathVariable("temperatura") Float temperatura){
		try{
			...
			List<Localita> locCalde=localitaService.ricercaByTemperaturaOver(temperatura);
	...
	public ResponseEntity<Localita> inserisci(@RequestBody Localita l){
		try {
			Localita loc=localitaService.ricercaById(l.getId());
			HttpStatus httpStatus= null;
			if(loc!=null) {
				httpStatus= HttpStatus.BAD_REQUEST;
			}else {
				httpStatus= HttpStatus.CREATED;
				localitaService.insert(l.getId(),l.getNome(),l.getTemperatura());
			}
			return new ResponseEntity<Localita>(l, httpStatus);
		...
		try {
			...
			if(loc!=null) {
				...
				if(l.getNome()!=null&& !l.getNome().isEmpty()) {
					loc.setNome(l.getNome());
				}
				if(l.getTemperatura()!=null&& !l.getTemperatura().isNaN()) {
					loc.setTemperatura(l.getTemperatura());
				}
				return new ResponseEntity<Localita>(l, httpStatus);
			}else {
				httpStatus= HttpStatus.NOT_FOUND;
	...
	public ResponseEntity<Void> cancella(@PathVariable("id") Integer id) {
		try {
			...
			Localita loc=localitaService.ricercaById(id);
			if(loc==null) {
	...
	public ResponseEntity<Void> cancellaTempUnder(@PathVariable("temperatura") Float temperatura) {
		try {
			...
			List<Localita> locFredde=localitaService.ricercaByTemperaturaUnder(temperatura);
			if(locFredde.isEmpty()) {
				return new ResponseEntity<Void>(HttpStatus.NOT_FOUND);
			}
			localita.removeAll(locFredde);
			return new ResponseEntity<Void>(HttpStatus.NO_CONTENT);
